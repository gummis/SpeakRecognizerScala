 ...
 * the License.
 */
package sly.speakrecognizer.test.math;
 ...
 * {@link MultivariateNormalMixtureExpectationMaximizationFitter}.
 */
public class MultivariateNormalMixtureExpectationMaximizationFitterTest {

    // TODO reject initial mixes where means/covMats not compatable with data
    // numCols
    ...
    public void testIncompatibleIntialMixture() {
        ...
        // data
        MultivariateNormalDistribution[] mvns =
                new MultivariateNormalDistribution[2];

        mvns[0] =
                new MultivariateNormalDistribution(new double[] {
    ...
    public void testInitialMixture() {
        ...
        MultivariateNormalDistribution[] correctMVNs =
                new MultivariateNormalDistribution[2];
    ...
    private void assertMultivariateNormalDistribution(MultivariateNormalDistribution expected, MultivariateNormalDistribution actual, double delta){
    ...
    public void testFit() {
        ...
        double[] correctWeights =
                new double[] { 0.2962324189652912, 0.7037675810347089 };
        MultivariateNormalDistribution[] correctMVNs =
                new MultivariateNormalDistribution[2];
        correctMVNs[0] =
                new MultivariateNormalDistribution(new double[] {
        ...
        Assert.assertEquals(correctLogLikelihood, fitter.getLogLikelihood(),
                Math.ulp(1d));
    ...
    public void testFitForDifferentSizesData() {
    	...
    	double[][][] covariances = {
    		new double[][]{
    			new double[]{1.74,-0.59},
    			new double[]{-0.59,1.02},
    		},
    		new double[][]{
        		new double[]{4.24,2.58},
        		new double[]{2.58,3.92},
    		},
    	};
    	double[][] means = {
       			new double[]{-1.42,1.69},
       			new double[]{4.21,7.98},
        	};
    	
    	double[] weights = new double[]{0.296,0.704};
    	MixtureMultivariateNormalDistribution mmnd = new MixtureMultivariateNormalDistribution(weights, means, covariances);

    	int[] lengths = new int[]{10,50,250,1500,10000,100000};
    	double[] errors = new double[lengths.length];
    	int counter = 0;
    	for(int length : lengths){
    		System.out.println ("Dla długości danych: " + length);
    	...
    	for(int x =  0; x < lengths.length ; x++){
    		System.out.println("Dla length = " + lengths[x] + " błąd wynosi = " + errors[x]);
    	}
    	
    }
    
    public static void printMMND(MixtureMultivariateNormalDistribution mmnd){
    	System.out.println("MixtureMultivariateNormalDistribution");
    	int counter = 0;
    	for(Pair<Double, MultivariateNormalDistribution> pair : mmnd.getComponents()){
        	System.out.println("MND " + ++counter);
        	System.out.println(" Weight: " + pair.getFirst());
        	System.out.println(" Means: " + Arrays.toString(pair.getSecond().getMeans()));
        	System.out.println(" StandardDeviations: " + Arrays.toString(pair.getSecond().getStandardDeviations()));
        	System.out.println(" Covariance: " );
        	for(double[] covR : pair.getSecond().getCovariances().getData()){
        		System.out.println(" : " + Arrays.toString(covR));
        	}
        	pair.getSecond().getCovariances().getData();
    	}
    	System.out.println("End MixtureMultivariateNormalDistribution");
    }
    
    public double[][] getErrors(double[] data,double[] pattern){
    	double[] error = new double[pattern.length];
    	double sumError = 0.0;
    	for(int x = 0 ; x < pattern.length ; x++){
    		double err = data[x] - pattern[x];
    		sumError += Math.abs(err);
    		error[x] = err;
    	}
    	return new double[][] {error, new double[]{sumError}};
    }
    
    
    public double printErrorsMMND(MixtureMultivariateNormalDistribution pattern, MixtureMultivariateNormalDistribution mmnd){
    	System.out.println("ErrorsMixtureMultivariateNormalDistribution");
    	double sumErrors = 0.0;
    	double error;
    	double[][] error2;
    	
    	for(int counter = 0 ; counter < pattern.getComponents().size() ; counter++){
    		Pair<Double, MultivariateNormalDistribution> pairPattern = pattern.getComponents().get(counter);
    		Pair<Double, MultivariateNormalDistribution> pair = mmnd.getComponents().get(counter);
        	System.out.println("MND " + ++counter);
        	
        	error = pair.getFirst() - pairPattern.getFirst();
        	sumErrors += Math.abs(error);
        	System.out.println(" WeightError: " + error);

        	error2 = getErrors(pair.getSecond().getMeans(),pairPattern.getSecond().getMeans());
        	sumErrors += error2[1][0];
        	System.out.println(" MeansError: " + Arrays.toString(error2[0]));
        	
        	error2 = getErrors(pair.getSecond().getStandardDeviations(),pairPattern.getSecond().getStandardDeviations());
        	sumErrors += error2[1][0];
        	System.out.println(" StandardDeviationsError: " + Arrays.toString(error2[0]));
        	
        	System.out.println(" CovarianceError: " );
        	for(int counter1 = 0 ; counter1 < pair.getSecond().getDimension() ; counter1++){
        		double[] covPattern = pairPattern.getSecond().getCovariances().getRow(counter1);
        		double[] cov = pair.getSecond().getCovariances().getRow(counter1);
        		
            	error2 = getErrors(cov,covPattern);
            	sumErrors += error2[1][0];
            	System.out.println(" : " + Arrays.toString(error2[0]));
        	}
    	}
    	System.out.println("Calkowity blad: " + sumErrors);
    	System.out.println("End ErrorsMixtureMultivariateNormalDistribution");
    	return sumErrors;
    }
    ...
    private double[][] getTestSamples(int length, MixtureMultivariateNormalDistribution mmnd){
		return mmnd.sample(length);
    }
    ...
    private double[][] getTestSamples() {
        ...
        // [4, 8.2]
        return new double[][] { 
        		{ 7.358553610469948, 11.31260831446758 },
